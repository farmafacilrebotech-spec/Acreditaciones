...
import { saveBackground } from '../services/backgroundStore.js';
...
router.post('/upload-background', upload.single('file'), async (req, res, next) => {
  try {
    if (!req.file) {
      return res.status(400).json({ error: 'Background file is required.' });
    }
    const processed = await sharp(req.file.buffer).png().toBuffer();
    const backgroundId = saveBackground(processed, 'image/png');
